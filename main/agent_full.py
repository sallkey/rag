"""
agent-agent_full.py - 七篇合一的完整 Agent
...
用法:
  python agent/full/agent-agent_full.py "你的任务"
  python agent/full/agent-agent_full.py --auto "你的任务"      # 跳过用户确认
  python agent/full/agent-agent_full.py --team "你的任务"       # 使用多智能体团队模式
"""
...
# ======================== 主编排 ========================
def build_system_prompt(rag_chunks: List[str]):
    """构建 system prompt: 基础指令 + Rules + Skills + Memory + rag"""
    parts = [
    ...
    if rag_chunks:
        ...
        for i, chunk in enumerate(rag_chunks):
            # 构建Prompt是一门学问，当结果不理想时，可以先尝试调整下prompt。
            parts.append(f"参考文档：\n{i + 1}. {chunk}\n\n")
    return "\n".join(parts)


def run_single(task, rag_chunks: List[str]):
    ...
    messages = [
        {"role": "system", "content": build_system_prompt(rag_chunks)},
        {"role": "user", "content": task},
    ]
...
#
#     if len(sys.argv) < 2:
#         print("Usage: python agent/full/agent-agent_full.py [--auto] [--team] 'your task'")
#         print()
#         print("Modes:")
...
#         print("Example:")
#         print(
#             "  python agent/full/agent-agent_full.py '列出当前目录的 Python 文件并统计行数'"
#         )
#         print(
#             "  python agent/full/agent-agent_full.py --team '创建一个 TODO 应用，包含后端和前端'"
#         )
#         sys.exit(1)
#
